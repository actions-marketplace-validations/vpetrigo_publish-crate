...
|            `path`            |          `.`          | Path to crate or workspace                                                  |
|       `registry-token`       |           -           | Cargo registry token for authentication                                     |
|            `args`            |           -           | Extra arguments passed to `cargo publish`                                   |
|          `dry-run`           |        `false`        | If `true`, performs a dry run without publishing                            |
|         `check-repo`         |        `true`         | If `true`, checks whether packages have changed since last publish          |
|       `publish-delay`        |           -           | Delay in milliseconds between publishing each crate                         |
|         `no-verify`          |        `false`        | If `true`, passes `--no-verify` to skip packaging verification              |
| `ignore-unpublished-changes` |        `false`        | If `true`, exits gracefully when no changes are detected instead of failing |
